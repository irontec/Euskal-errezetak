...
/**
 * Data Mapper implementation for EuskalErrezetak\Model\Tags
 *
 * @package EuskalErrezetak\Mapper\Sql
 * @subpackage Raw
 * @author Mikel Eizagirre
...
class Tags extends MapperAbstract
{
    protected $_modelName = 'EuskalErrezetak\\Model\\Tags';
     ...
     *
     * @param EuskalErrezetak\Model\Raw\Tags $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\Tags) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \EuskalErrezetak\Model\Raw\Tags object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\Model\\Tags object in toArray for " . get_class($this);
            }
     ...
     *
     * @return EuskalErrezetak\\Mapper\\Sql\\DbTable\\Tags
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('EuskalErrezetak\\Mapper\\Sql\\DbTable\\Tags');
        }
     ...
     *
     * @param EuskalErrezetak\Model\Raw\Tags $model The model to delete
    ...
    public function delete(\EuskalErrezetak\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\Tags) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\EuskalErrezetak\\Model\\Tags object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\\Model\\Tags object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('EuskalErrezetak\\Mapper\\Sql\\DbTable\\Tags', $capitalizedFk);
     ...
     */
    public function save(\EuskalErrezetak\Model\Raw\Tags $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\EuskalErrezetak\Model\Raw\Tags $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\EuskalErrezetak\Model\Raw\Tags $model,
     ...
     * @param EuskalErrezetak\Model\Raw\Tags|null $entry The object to load the data into, or null to have one created
     * @return EuskalErrezetak\Model\Raw\Tags The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \EuskalErrezetak\Model\Tags();
        }
