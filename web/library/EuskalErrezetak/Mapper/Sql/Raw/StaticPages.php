...
/**
 * Data Mapper implementation for EuskalErrezetak\Model\StaticPages
 *
 * @package EuskalErrezetak\Mapper\Sql
 * @subpackage Raw
 * @author Mikel Eizagirre
...
class StaticPages extends MapperAbstract
{
    protected $_modelName = 'EuskalErrezetak\\Model\\StaticPages';
     ...
     *
     * @param EuskalErrezetak\Model\Raw\StaticPages $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\StaticPages) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \EuskalErrezetak\Model\Raw\StaticPages object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\Model\\StaticPages object in toArray for " . get_class($this);
            }
     ...
     *
     * @return EuskalErrezetak\\Mapper\\Sql\\DbTable\\StaticPages
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('EuskalErrezetak\\Mapper\\Sql\\DbTable\\StaticPages');
        }
     ...
     *
     * @param EuskalErrezetak\Model\Raw\StaticPages $model The model to delete
    ...
    public function delete(\EuskalErrezetak\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\StaticPages) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\EuskalErrezetak\\Model\\StaticPages object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\\Model\\StaticPages object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('EuskalErrezetak\\Mapper\\Sql\\DbTable\\StaticPages', $capitalizedFk);
     ...
     */
    public function save(\EuskalErrezetak\Model\Raw\StaticPages $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\EuskalErrezetak\Model\Raw\StaticPages $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\EuskalErrezetak\Model\Raw\StaticPages $model,
     ...
     * @param EuskalErrezetak\Model\Raw\StaticPages|null $entry The object to load the data into, or null to have one created
     * @return EuskalErrezetak\Model\Raw\StaticPages The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \EuskalErrezetak\Model\StaticPages();
        }
        ...
        } else if ($data instanceof \EuskalErrezetak\Model\Raw\StaticPages) {
