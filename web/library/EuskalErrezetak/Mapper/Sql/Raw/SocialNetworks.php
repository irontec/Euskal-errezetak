...
/**
 * Data Mapper implementation for EuskalErrezetak\Model\SocialNetworks
 *
 * @package EuskalErrezetak\Mapper\Sql
 * @subpackage Raw
 * @author Mikel Eizagirre
...
class SocialNetworks extends MapperAbstract
{
    protected $_modelName = 'EuskalErrezetak\\Model\\SocialNetworks';
     ...
     *
     * @param EuskalErrezetak\Model\Raw\SocialNetworks $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\SocialNetworks) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \EuskalErrezetak\Model\Raw\SocialNetworks object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\Model\\SocialNetworks object in toArray for " . get_class($this);
            }
        ...
        $result = array(
            'id' => $model->getId(),
            'title_es' => $model->getTitleEs(),
            'title_eu' => $model->getTitleEu(),
            'url_es' => $model->getUrlEs(),
            'url_eu' => $model->getUrlEu(),
     ...
     *
     * @return EuskalErrezetak\\Mapper\\Sql\\DbTable\\SocialNetworks
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('EuskalErrezetak\\Mapper\\Sql\\DbTable\\SocialNetworks');
        }
     ...
     *
     * @param EuskalErrezetak\Model\Raw\SocialNetworks $model The model to delete
    ...
    public function delete(\EuskalErrezetak\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\SocialNetworks) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\EuskalErrezetak\\Model\\SocialNetworks object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\\Model\\SocialNetworks object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('EuskalErrezetak\\Mapper\\Sql\\DbTable\\SocialNetworks', $capitalizedFk);
     ...
     */
    public function save(\EuskalErrezetak\Model\Raw\SocialNetworks $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\EuskalErrezetak\Model\Raw\SocialNetworks $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\EuskalErrezetak\Model\Raw\SocialNetworks $model,
     ...
     * @param EuskalErrezetak\Model\Raw\SocialNetworks|null $entry The object to load the data into, or null to have one created
     * @return EuskalErrezetak\Model\Raw\SocialNetworks The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \EuskalErrezetak\Model\SocialNetworks();
        }
        ...
        if (is_array($data)) {
            $entry->setId($data['id'])
                  ...
                  ->setTitleEu($data['title_eu'])
                  ->setUrlEs($data['url_es'])
                  ->setUrlEu($data['url_eu'])
        ...
        } else if ($data instanceof \EuskalErrezetak\Model\Raw\SocialNetworks) {
            $entry->setId($data->getId())
                  ->setTitleEs($data->getTitleEs())
                  ->setTitleEu($data->getTitleEu())
                  ->setUrlEs($data->getUrlEs())
                  ->setUrlEu($data->getUrlEu())
