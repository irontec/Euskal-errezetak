...
class RecipeCategory extends MapperAbstract
{
    protected $_modelName = 'EuskalErrezetak\\Model\\RecipeCategory';
     ...
     *
     * @param EuskalErrezetak\Model\Raw\RecipeCategory $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\RecipeCategory) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \EuskalErrezetak\Model\Raw\RecipeCategory object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\Model\\RecipeCategory object in toArray for " . get_class($this);
            }
     ...
     *
     * @return EuskalErrezetak\\Mapper\\Sql\\DbTable\\RecipeCategory
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('EuskalErrezetak\\Mapper\\Sql\\DbTable\\RecipeCategory');
        }
     ...
     *
     * @param EuskalErrezetak\Model\Raw\RecipeCategory $model The model to delete
    ...
    public function delete(\EuskalErrezetak\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\RecipeCategory) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\EuskalErrezetak\\Model\\RecipeCategory object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\\Model\\RecipeCategory object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('EuskalErrezetak\\Mapper\\Sql\\DbTable\\RecipeCategory', $capitalizedFk);
     ...
     */
    public function save(\EuskalErrezetak\Model\Raw\RecipeCategory $model)
    {
     ...
     * @return integer primary key for autoincrement fields if the save action was successful
     */
    public function saveRecursive(\EuskalErrezetak\Model\Raw\RecipeCategory $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\EuskalErrezetak\Model\Raw\RecipeCategory $model,
     ...
     * @param EuskalErrezetak\Model\Raw\RecipeCategory|null $entry The object to load the data into, or null to have one created
     * @return EuskalErrezetak\Model\Raw\RecipeCategory The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \EuskalErrezetak\Model\RecipeCategory();
        }
