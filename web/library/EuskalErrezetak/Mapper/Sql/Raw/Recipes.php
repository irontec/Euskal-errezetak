...
class Recipes extends MapperAbstract
{
    protected $_modelName = 'EuskalErrezetak\\Model\\Recipes';
    ...
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\Recipes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \EuskalErrezetak\Model\Raw\Recipes object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\Model\\Recipes object in toArray for " . get_class($this);
            }
     ...
     *
     * @return EuskalErrezetak\\Mapper\\Sql\\DbTable\\Recipes
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('EuskalErrezetak\\Mapper\\Sql\\DbTable\\Recipes');
        }
     ...
     *
     * @param EuskalErrezetak\Model\Raw\Recipes $model The model to delete
    ...
    public function delete(\EuskalErrezetak\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\Recipes) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\EuskalErrezetak\\Model\\Recipes object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\\Model\\Recipes object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('EuskalErrezetak\\Mapper\\Sql\\DbTable\\Recipes', $capitalizedFk);
    ...
    protected function _save(\EuskalErrezetak\Model\Raw\Recipes $model,
     ...
     * @param EuskalErrezetak\Model\Raw\Recipes|null $entry The object to load the data into, or null to have one created
     * @return EuskalErrezetak\Model\Raw\Recipes The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \EuskalErrezetak\Model\Recipes();
        }
