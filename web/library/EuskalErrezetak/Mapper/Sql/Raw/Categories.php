...
/**
 * Data Mapper implementation for EuskalErrezetak\Model\Categories
 *
 * @package EuskalErrezetak\Mapper\Sql
 * @subpackage Raw
 * @author Mikel Eizagirre
...
class Categories extends MapperAbstract
{
    protected $_modelName = 'EuskalErrezetak\\Model\\Categories';
    ...
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\Categories) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \EuskalErrezetak\Model\Raw\Categories object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\Model\\Categories object in toArray for " . get_class($this);
            }
     ...
     *
     * @return EuskalErrezetak\\Mapper\\Sql\\DbTable\\Categories
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('EuskalErrezetak\\Mapper\\Sql\\DbTable\\Categories');
        }
     ...
     *
     * @param EuskalErrezetak\Model\Raw\Categories $model The model to delete
    ...
    public function delete(\EuskalErrezetak\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\Categories) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\EuskalErrezetak\\Model\\Categories object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\\Model\\Categories object in delete for " . get_class($this);
            }
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('EuskalErrezetak\\Mapper\\Sql\\DbTable\\Categories', $capitalizedFk);
    ...
    protected function _save(\EuskalErrezetak\Model\Raw\Categories $model,
     ...
     * @param EuskalErrezetak\Model\Raw\Categories|null $entry The object to load the data into, or null to have one created
     * @return EuskalErrezetak\Model\Raw\Categories The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \EuskalErrezetak\Model\Categories();
        }
