...
/**
 * Data Mapper implementation for EuskalErrezetak\Model\KlearUsers
 *
 * @package EuskalErrezetak\Mapper\Sql
 * @subpackage Raw
 * @author Mikel Eizagirre
...
class KlearUsers extends MapperAbstract
{
    protected $_modelName = 'EuskalErrezetak\\Model\\KlearUsers';
     ...
     *
     * @param EuskalErrezetak\Model\Raw\KlearUsers $model
     * @return array
     */
    public function toArray($model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \EuskalErrezetak\Model\Raw\KlearUsers object in toArray for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\Model\\KlearUsers object in toArray for " . get_class($this);
            }
     ...
     *
     * @return EuskalErrezetak\\Mapper\\Sql\\DbTable\\KlearUsers
     */
    public function getDbTable()
    {
        if (is_null($this->_dbTable)) {
            $this->setDbTable('EuskalErrezetak\\Mapper\\Sql\\DbTable\\KlearUsers');
        }
     ...
     *
     * @param EuskalErrezetak\Model\Raw\KlearUsers $model The model to delete
    ...
    public function delete(\EuskalErrezetak\Model\Raw\ModelAbstract $model)
    {
        if (!$model instanceof \EuskalErrezetak\Model\Raw\KlearUsers) {
            if (is_object($model)) {
                $message = get_class($model) . " is not a \\EuskalErrezetak\\Model\\KlearUsers object in delete for " . get_class($this);
            } else {
                $message = "$model is not a \\EuskalErrezetak\\Model\\KlearUsers object in delete for " . get_class($this);
            }
        ...
        try {
                ...
                foreach ($deleteCascade as $fk) {
                    ...
                    if (!isset($depList[$capitalizedFk])) {
                        ...
                        if ( class_exists($relDbAdapName) && class_exists($depModelName) ) {
                            ...
                            $references = $relDbAdapter->getReference('EuskalErrezetak\\Mapper\\Sql\\DbTable\\KlearUsers', $capitalizedFk);
            ...
            $where = $dbAdapter->quoteInto($dbAdapter->quoteIdentifier('userId') . ' = ?', $model->getUserId());
     ...
     */
    public function save(\EuskalErrezetak\Model\Raw\KlearUsers $model)
    {
     ...
     * @return boolean If the save action was successful
     */
    public function saveRecursive(\EuskalErrezetak\Model\Raw\KlearUsers $model, $useTransaction = true, $transactionTag = null)
    {
        return $this->_save($model, true, $useTransaction, $transactionTag);
    }

    protected function _save(\EuskalErrezetak\Model\Raw\KlearUsers $model,
            ...
            if (is_null($primaryKey) || empty($primaryKey)) {
                ...
                $this->getDbTable()
                     ->update(
                         $data,
                         array(
                             $this->getDbTable()->getAdapter()->quoteIdentifier('userId') . ' = ?' => $primaryKey
                         )
                     );
            }
     ...
     * @param EuskalErrezetak\Model\Raw\KlearUsers|null $entry The object to load the data into, or null to have one created
     * @return EuskalErrezetak\Model\Raw\KlearUsers The model with the data provided
     */
    public function loadModel($data, $entry = null)
    {
        if (!$entry) {
            $entry = new \EuskalErrezetak\Model\KlearUsers();
        }
        ...
        } else if ($data instanceof \EuskalErrezetak\Model\Raw\KlearUsers) {
